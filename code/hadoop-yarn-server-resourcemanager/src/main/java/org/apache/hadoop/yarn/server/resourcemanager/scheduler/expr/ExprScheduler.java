...
public class ExprScheduler extends
        AbstractYarnScheduler<FiCaSchedulerApp, FiCaSchedulerNode> implements
        Configurable {

    private static final Log LOG = LogFactory.getLog(ExprScheduler.class);
    ...
    private List<SchedulerApplication<FiCaSchedulerApp>> running = new ArrayList<>();
    ...
    public ExprScheduler() {
        super(ExprScheduler.class.getName());
    }
    ...
    private void assignContainers(FiCaSchedulerNode node) {
        ...
        for (AppInfo appInfo: order) {
            ApplicationId applicationId = ApplicationId.newInstance(appInfo.getClusterTimestamp(), appInfo.getId());
            FiCaSchedulerApp application = applications.get(applicationId).getCurrentAppAttempt();
...
//        }
        ...
        if (assigned) OutputToFile.addCount();
    ...
    private List<AppInfo> sendRequestAndGetOrder(FiCaSchedulerNode node) {
        ...
        List<AppInfo> appInfos = new ArrayList<>(); // 存放结果
        for (Map.Entry<ApplicationId, SchedulerApplication<FiCaSchedulerApp>> e : applications.entrySet()) {
            ...
            for (Priority priority : app.getPriorities()) {
                ...
                if (requests != null) {
                    for (ResourceRequest request : requests.values()) {
                        ...
                        appInfos.add(new AppInfo(id, maxMemory, maxCpu, clusterTimestamp));
                    }
                }
            }
        }
        if (appInfos.size() == 0) return new ArrayList<AppInfo>();
        ...
        // 调用Python
        List<AppInfo> order = SocketClient.sendRequireData(availableMemory, availableVCPU, appInfos);
