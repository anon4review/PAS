...
public abstract class AbstractYarnScheduler
  ...
  class SendResourceThread implements Runnable {
    private ResourceScheduler rs;

    public SendResourceThread(ResourceScheduler rs) {
      this.rs = rs;
    }
    ...
    public void run() {
      while (true) {
        ...
        long reservedVirtualCores = metrics.getReservedVirtualCores();
        long availableVirtualCores = metrics.getAvailableVirtualCores();
        long allocatedVirtualCores = metrics.getAllocatedVirtualCores();

        if (applications.size() > 0)
          OutputToFile.outputResourceInfo(reservedMB, availableMB, allocatedMB,
  ...
  public void serviceInit(Configuration conf) throws Exception {
    ...
    new Thread(new SendResourceThread(this)).start();
  }
