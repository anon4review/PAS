...
package org.apache.hadoop.yarn.server.resourcemanager.scheduler.expr1;
...
import org.apache.hadoop.yarn.server.resourcemanager.scheduler.expr.AppInfo;
import org.apache.hadoop.yarn.server.resourcemanager.scheduler.agentutil.OutputToFile;
import org.apache.hadoop.yarn.server.resourcemanager.scheduler.expr.SocketClient;
...
public class Expr1Scheduler extends
        AbstractYarnScheduler<FiCaSchedulerApp, FiCaSchedulerNode> implements
        Configurable {

    private static final Log LOG = LogFactory.getLog(Expr1Scheduler.class);
    ...
    private List<SchedulerApplication<FiCaSchedulerApp>> runningQuene = new ArrayList<>();
    private Set<Integer> runningId = new HashSet<>();
    ...
    public Expr1Scheduler() {
        super(Expr1Scheduler.class.getName());
    }
    ...
    private synchronized void doneApplication(ApplicationId applicationId,
        ...
        // 删除已完成的应用
        runningQuene.remove(application);
    ...
    private void assignContainers(FiCaSchedulerNode node) {
        ...
        for (SchedulerApplication<FiCaSchedulerApp> app : runningQuene) {
//            LOG.info("MyLOG:  Earlier Assigned App Id = " + app.getCurrentAppAttempt().getApplicationId().getId() );
            assignContainerToAM(app, node);
...
//            LOG.info("MyLOG:  Earlier Assign End!");
        }


        if (!Resources.lessThan(resourceCalculator, getClusterResource(),
                node.getAvailableResource(), minimumAllocation)) {
            List<AppInfo> order = sendRequestAndGetOrder(node);

            for (AppInfo appInfo: order) {
                SchedulerApplication<FiCaSchedulerApp> app = applications.get(ApplicationId.newInstance(appInfo.getClusterTimestamp(), appInfo.getId()));
                if (app == null || app.getCurrentAppAttempt() == null) continue;

//                LOG.info("MyLOG:  Current Assigned App Id = " + app.getCurrentAppAttempt().getApplicationId().getId() );
                runningQuene.add(app);
                runningId.add(appInfo.getId());
                assignContainerToAM(app, node);
            }

//            if (order.size() != 0)
//                LOG.info("MyLOG:  Current Assign End! assigned AM = " + order.size());
        }

        OutputToFile.addCount();
    ...
    private void assignContainerToAM(SchedulerApplication<FiCaSchedulerApp> app, FiCaSchedulerNode node) {
        ...
        synchronized (application) {
            ...
            // 一个应用要求很多优先级？依次判断是否有满足各个优先级的Container？ *****
...
//                    LOG.info(String.format("MyLOG:  ID = %d, RM Assigned Containers = %d",
...
//                            assignedContainers));
                    ...
                    OutputToFile.outputOrderInfo(node, application, priority, assignedContainers);
                }
            }
        }
    ...
    private List<AppInfo> sendRequestAndGetOrder(FiCaSchedulerNode node) {
        String nodeId = node.getNodeID().getHost();
//        int availableMemory = node.getAvailableResource().getMemory();
//        int availableVCPU = node.getAvailableResource().getVirtualCores();

        Resource availResource = Resources.subtract(getClusterResource(), getUsedResource());
        int availableMemory = availResource.getMemory();
        int availableVCPU = availResource.getVirtualCores();

        List<AppInfo> waitingAppInfo = new ArrayList<>(); // 存放结果
        List<AppInfo> runningAppInfo = new ArrayList<>();
        for (Map.Entry<ApplicationId, SchedulerApplication<FiCaSchedulerApp>> e : applications.entrySet()) {
            if (e.getValue().getCurrentAppAttempt() == null) continue;

            String appName = rmContext.getRMApps().get(e.getKey()).getName();
            int id = e.getKey().getId();
            long clusterTimestamp = e.getKey().getClusterTimestamp();
            FiCaSchedulerApp app = e.getValue().getCurrentAppAttempt();

            if (runningId.contains(e.getKey().getId())) {
                runningAppInfo.add(new AppInfo(id, 0, 0, clusterTimestamp, 0, appName));
                continue;
            }
...
//                LOG.info(String.format("MyLOG:  Max Containers = %d, Priority = %d", maxContainers, priority.getPriority()));

                if (maxContainers <= 0) continue;
                ...
                if (requests != null) {
                    for (ResourceRequest request : requests.values()) {
                        ...
                        waitingAppInfo.add(new AppInfo(id, 0, 0, clusterTimestamp, 0, appName));
//                        LOG.info("MyLOG:  App Name = " + appName);
                        break;

//                        // 容器想要的内存
//                        int containerMemory = request.getCapability().getMemory();
//                        // 容器想要的CPU
//                        int containerCpu = request.getCapability().getVirtualCores();
//
//                        // 最多可分配的容器数量
//                        int maxAllocatableContainer = Math.min(Math.min(availableMemory / containerMemory, availableVCPU / containerCpu),
//                                numContainers);
//
//                        if (maxAllocatableContainer == 0) continue;
//
//                        // 本次最多可分配的Memory
//                        int maxMemory = containerMemory * maxAllocatableContainer;
//                        // 本次最多可分配的CPU
//                        int maxCpu = containerCpu * maxAllocatableContainer;
//
//                        LOG.info(String.format("MyLOG:  ID = %d, Priority = %d, Num Container = %d, \n" +
//                                        "\tContainer Memory = %d, Container CPU = %d, \n" +
//                                        "\tMax Allocatable Memory = %d, Max Allocatable CPU = %d \n" +
//                                        "\tRequestType = %s, Max Allocatable Container = %d",
//                                id, priority.getPriority(), numContainers,
//                                containerMemory, containerCpu,
//                                maxMemory, maxCpu,
//                                request.getResourceName(), maxAllocatableContainer));
//
//                        OutputToFile.outputQueueAppInfo(node, app, request, maxAllocatableContainer);
                    }
                }
            }
        }
        if (waitingAppInfo.size() == 0) return new ArrayList<AppInfo>();

//        LOG.info(String.format("MyLOG:  Node ID = %s, " +
//                        "Node Aval Memory = %d, Node Aval CPU = %d",
//                nodeId, availableMemory, availableVCPU));

        // 调用Python
        List<AppInfo> order = SocketClient.sendRequireData(availableMemory, availableVCPU, waitingAppInfo, runningAppInfo);
        LOG.info("MyLOG:  The length of order is " + order.size());
...
//            LOG.info(String.format("MyLOG:  Get Max Allocatable = %d, RequestType = %s",
//                    offSwitchRequest.getNumContainers(),
//                    offSwitchRequest.getResourceName()));
        }
...
//            LOG.info(String.format("MyLOG:      Assigned LOCAL = %d", assignableContainers));
        }
